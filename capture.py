...
CAP_EXTENSIONS  = (".cap", ".pcap", ".pcapng", ".ivs")
HASH_EXTENSIONS = (".hccapx", ".22000", ".hc22000")


# ─── VALIDATE CAP FILE ──────────────────────────────────
def is_cap_file(path):
    """Check if path exists and has a known capture/hash extension."""
    if not os.path.isfile(path):
        return False
    ext = os.path.splitext(path)[1].lower()
    return ext in CAP_EXTENSIONS + HASH_EXTENSIONS


def is_hash_file(path):
    """Already-converted hash file (hashcat-ready)."""
    if not os.path.isfile(path):
        return False
    return os.path.splitext(path)[1].lower() in HASH_EXTENSIONS
...
def cap2hccapx_available():
    return (shutil.which("cap2hccapx") is not None or
            shutil.which("hcxpcaptool") is not None)


# ─── PARSE INFO FROM CAP (aircrack-ng) ──────────────────
def parse_cap_info(cap_path):
    """
    Run: aircrack-ng <cap>
    Parse BSSID, ESSID, encryption, channel from output.
    Returns dict with keys: bssid, essid, encryption, channel, handshake, raw
    """
    info = {
        "bssid":      None,
        "essid":      None,
...
def run_hashcat(hash_file, wordlist_path, progress_cb=None):
    ...
    FIXED v6.0: found password is captured reliably via --outfile
    instead of guessing from the potfile.
    Returns (found_password_or_None, full_output).
    NOTE: hash_file must be in .hccapx / .22000 format.
    """
    if not hashcat_available():
        return None, "[!] hashcat not installed."

    outfile = tempfile.NamedTemporaryFile(
        mode="w+", suffix=".out", delete=False).name
    ...
    # reliable found-detection: read the outfile
    found = None
    try:
    ...
    # fallback: potfile (best effort, only if outfile gave nothing)
    if found is None:
...
def convert_cap_to_hccapx(cap_path, out_path=None):
    """
    Convert .cap file to .hccapx using cap2hccapx / hcxpcaptool.
